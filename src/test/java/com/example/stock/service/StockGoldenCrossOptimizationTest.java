...
class StockGoldenCrossOptimizationTest {
    ...
    void testGoldenCrossOptimization() {
        ...
        // 统计各类条件的结果数
        long condition1 = results.stream().filter(r -> r.getMatchedCondition().contains("低于历史最高�?)).count();
        ...
        long condition5 = results.stream().filter(r -> r.getMatchedCondition().contains("成交量激�?)).count();
        long condition6 = results.stream().filter(r -> r.getMatchedCondition().contains("均线金叉")).count();

        System.out.println("📈 各条件结果统�?");
        System.out.println("   条件1 (跌幅�?5%): " + condition1 + " �?);
        System.out.println("   条件2 (高波动低�?: " + condition2 + " �?);
        System.out.println("   条件3 (连续上涨): " + condition3 + " �?);
        System.out.println("   条件4 (接近年度高点): " + condition4 + " �?);
        System.out.println("   条件5 (成交量激�?: " + condition5 + " �?);
        System.out.println("   条件6 (均线金叉): " + condition6 + " �?);
        ...
        // 验证至少有一个金叉股�?
        assertTrue(condition6 > 0, "应该至少找到一个金叉股�?);
        ...
        assertNotNull(goldenCrossStock, "应该找到至少一个金叉股�?);
        assertNotNull(goldenCrossStock.getSymbol(), "股票代码不应为null");
        assertNotNull(goldenCrossStock.getSymbol(), "股票名称不应为null");
        assertTrue(goldenCrossStock.getCurrentPrice() > 0, "当前价格应大�?");

        System.out.println("📋 金叉股票示例:");
        System.out.println("   股票代码: " + goldenCrossStock.getSymbol());
        System.out.println("   股票名称: " + goldenCrossStock.getSymbol());
