...
class StockAnalysisServiceTest {
    ...
    void testFindStocksBelowHistoricalHigh() {
        ...
        results.stream().limit(10).forEach(r -> {
            System.out.println(String.format("   - %s %s | 当前价: %.2f | 历史最高: %.2f | 跌幅: %.2f%%",
                    r.getSymbol(), r.getName(), r.getCurrentPrice(), 
        ...
        if (!results.isEmpty()) {
            ...
            assertEquals("低于历史最高值25.0%以上", firstResult.getMatchedCondition());
        }
    }
     ...
     * 
     * @Test 参数说明：
     * - startDate: 开始日期，格式为 YYYY-MM-DD（可选，默认为全部历史数据）
     * - dropPercentage: 跌幅百分比，范围 1-99（默认为25）
     */
    @Test
    void testFindStocksBelowHistoricalHighWithParams() {
        System.out.println("\n📊 测试条件1 (参数版): 超跌潜力股筛选 - 自定义参数");
        
        // 测试用例1：跌幅30%，2025年初以来
        System.out.println("\n📌 测试用例1: 跌幅>=30%，2025-01-01以来");
    ...
    void testFindContinuousRiseStocks() {
        ...
        results.stream().limit(10).forEach(r -> {
            System.out.println(String.format("   - %s %s | 当前价: %.2f | %s",
                    r.getSymbol(), r.getName(), r.getCurrentPrice(), r.getMatchedCondition()));
        });
    ...
    void testMultipleConditions() {
        ...
        results.stream().limit(10).forEach(r -> {
            System.out.println(String.format("   - %s %s | %s",
                    r.getSymbol(), r.getName(), r.getMatchedCondition()));
        });
    }
