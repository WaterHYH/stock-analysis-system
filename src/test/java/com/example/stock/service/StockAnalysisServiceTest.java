...
class StockAnalysisServiceTest {
    ...
    void testFindStocksBelowHistoricalHigh() {
        ...
        results.stream().limit(10).forEach(r -> {
            System.out.println(String.format("   - %s %s | 当前�? %.2f | 历史最�? %.2f | 跌幅: %.2f%%",
        ...
        if (!results.isEmpty()) {
            ...
            assertTrue(firstResult.getDropPercentage() >= 25.0, "跌幅应大于等�?5%");
            assertEquals("低于历史最高�?5.0%以上", firstResult.getMatchedCondition());
        }
    }
     ...
     * 
     * @Test 参数说明�?
     * - startDate: 开始日期，格式�?YYYY-MM-DD（可选，默认为全部历史数据）
     * - dropPercentage: 跌幅百分比，范围 1-99（默认为25�?
     */
    @Test
    void testFindStocksBelowHistoricalHighWithParams() {
        System.out.println("\n📊 测试条件1 (参数�?: 超跌潜力股筛�?- 自定义参�?);
        
        // 测试用例1：跌�?0%�?025年初以来
        System.out.println("\n📌 测试用例1: 跌幅>=30%�?025-01-01以来");
        ...
        System.out.println("�?找到 " + results1.size() + " 只跌�?=30%的超跌潜力股");
        ...
        if (!results1.isEmpty()) {
            ...
            assertTrue(firstResult.getDropPercentage() >= 30.0, "跌幅应大于等�?0%");
            assertTrue(firstResult.getMatchedCondition().contains("30.0%"), "条件描述应包�?0.0%");
        }
        
        // 测试用例2：跌�?0%，最近半�?
        System.out.println("\n📌 测试用例2: 跌幅>=20%�?024-05-01以来");
        ...
        System.out.println("�?找到 " + results2.size() + " 只跌�?=20%的超跌潜力股");
        ...
        if (!results2.isEmpty()) {
            ...
            assertTrue(firstResult.getDropPercentage() >= 20.0, "跌幅应大于等�?0%");
        }
        
        // 测试用例3：跌�?0%，全部历史数据（不指定开始日期）
        System.out.println("\n📌 测试用例3: 跌幅>=50%，全部历史数�?);
        ...
        System.out.println("�?找到 " + results3.size() + " 只跌�?=50%的超跌潜力股");
        ...
        if (!results3.isEmpty()) {
            ...
            assertTrue(firstResult.getDropPercentage() >= 50.0, "跌幅应大于等�?0%");
        }
        
        // 比较结果
        System.out.println("\n📊 参数对比总结:");
        System.out.println("   - 跌幅30%, 2025年初: " + results1.size() + " 只股�?);
        System.out.println("   - 跌幅20%, 2024年中: " + results2.size() + " 只股�?);
        System.out.println("   - 跌幅50%, 全部数据: " + results3.size() + " 只股�?);
        System.out.println("   - 跌幅越大，股票数量越�?�?);
    }
    ...
    void testFindHighVolatilityLowPriceStocks() {
        ...
        // 验证结果
        assertNotNull(results, "结果不应为null");
        System.out.println("�?找到 " + results.size() + " 只高波动低位�?);
        
        // 打印�?0条结�?
        results.stream().limit(10).forEach(r -> {
            System.out.println(String.format("   - %s %s | 当前�? %.2f | 波动次数: %d | 跌幅: %.2f%%",
                    r.getSymbol(), r.getSymbol(), r.getCurrentPrice(), 
        ...
        if (!results.isEmpty()) {
            ...
            assertTrue(firstResult.getVolatilityCount() >= 3, "波动次数应大于等�?");
            assertTrue(firstResult.getMatchedCondition().contains("高波�?), "条件描述应包含高波动");
        }
    }
    ...
    void testFindContinuousRiseStocks() {
        ...
        // 打印�?0条结�?
        results.stream().limit(10).forEach(r -> {
            System.out.println(String.format("   - %s %s | 当前�? %.2f | %s",
        ...
        if (!results.isEmpty()) {
            StockAnalysisDTO firstResult = results.get(0);
            assertTrue(firstResult.getMatchedCondition().contains("连续上涨"), "条件描述应包含连续上�?);
        }
    }
    ...
    void testFindNearYearHighStocks() {
        ...
        // 验证结果
        assertNotNull(results, "结果不应为null");
        System.out.println("�?找到 " + results.size() + " 只创新高潜力�?);
        
        // 打印�?0条结�?
        results.stream().limit(10).forEach(r -> {
            System.out.println(String.format("   - %s %s | 当前�? %.2f | 历史最�? %.2f | 距最�? %.2f%%",
                    r.getSymbol(), r.getSymbol(), r.getCurrentPrice(), 
                    r.getHistoricalHigh(), r.getDropPercentage()));
        });
    ...
    void testFindVolumeSurgeStocks() {
        ...
        // 验证结果
        assertNotNull(results, "结果不应为null");
        System.out.println("�?找到 " + results.size() + " 只成交量爆发�?);
        ...
        if (!results.isEmpty()) {
            StockAnalysisDTO firstResult = results.get(0);
            assertTrue(firstResult.getMatchedCondition().contains("成交量激�?), "条件描述应包含成交量激�?);
        }
    }
    ...
    void testFindGoldenCrossStocks() {
        ...
        System.out.println("⏱️  开始时�? " + startTime);
        
        // 执行筛�?
        long beforeCall = System.currentTimeMillis();
        System.out.println("⏱️  调用服务�? " + beforeCall + " (距离开�? " + (beforeCall - startTime) + "ms)");
        ...
        System.out.println("⏱️  调用服务�? " + afterCall + " (服务耗时: " + callTime + "ms)");
        ...
        if (!results.isEmpty()) {
            StockAnalysisDTO firstResult = results.get(0);
            assertTrue(firstResult.getMatchedCondition().contains("均线金叉"), "条件描述应包含均线金�?);
        }
    ...
    void testMultipleConditions() {
        ...
        // 验证结果
        assertNotNull(results, "结果不应为null");
        System.out.println("�?多条件筛选找�?" + results.size() + " 只股�?);
    ...
    void testEmptyConditions() {
        ...
        assertNotNull(results, "结果不应为null");
        assertTrue(results.isEmpty(), "空条件应返回空结�?);
        
        System.out.println("�?空条件处理正确，返回空列�?);
    }
    ...
    void testInvalidCondition() {
        ...
        assertNotNull(results, "结果不应为null");
        System.out.println("�?无效条件处理正确，返�?" + results.size() + " 条结�?);
    }
    ...
    void testAllConditions() {
        ...
        assertNotNull(results, "结果不应为null");
        System.out.println("�?全部条件筛选找�?" + results.size() + " 只股�?);
        ...
        results.stream()
            ...
            .forEach((condition, count) -> 
                System.out.println("   - " + condition + ": " + count + " 只股�?));
    }
}
